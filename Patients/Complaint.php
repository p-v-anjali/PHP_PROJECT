...
if(isset($_POST['btn_submit']))
	{
		...
		{
			$ins = "insert into tbl_complaint(complaint_content,complainttype_id,patient_id,complaint_date,complaint_time) values('".$complaint."','".$complainttype."','".$_SESSION["patientid"]."',curdate(),curtime())";
		echo $ins;
			if($con->query($ins))
			{
  ...
  <p>&nbsp;</p>
     ...
     <table border="1" cellpadding="11" align="center">
        <tr>
         ...
         <?php
	  $i=0;
	  $sel = "select * from tbl_complaint c inner join tbl_complainttype ct on ct.complainttype_id=c.complainttype_id where patient_id='".$_SESSION["patientid"]."'";
	  $row = $con->query($sel);
	  while($data = $row->fetch_assoc())
	  {
		  $i++;
