...
<?php
	...
	{
		$selq="select * from tbl_patient where patient_password='".$_POST["txtcurrent"]."' and patient_id='".$_SESSION["patientid"]."'";
		$row=$con->query($selq);
		if($data=$row->fetch_assoc())
		{
			if($_POST["txtnew"]==$_POST["txtcon"])
			{
				$up="update tbl_patient set patient_password='".$_POST["txtnew"]."' where patient_id='".$_SESSION["patientid"]."'";
				if($con->query($up))
		{
			?>
        <script>
